...
from model.yolov3_prac import yolo
...
class yolo_predictor:
    def __init__(self,obj_threshold, nms_threshold, classes_file, anchors_file):
        ...
        self.colors = list(map(lambda x: (int(x[0]*255), int(x[2]*255)), self.colors))
        random.seed(10101)
    ...
    def boxes_and_scores(self, feats, anchors, classes_num, input_shape, image_shape):
        ...
        # 獲得特徵
        box_xy, box_wh,box_confidence, box_clalss_probs = self._get_feats(feats, anchors, classes_num, input_shape)
        # 尋找在原圖上的位置
        boxes = self.correct_boxes(box_xy, box_wh, input_shape, image_shape)
        boxes = tf.reshape(boxes,[-1, 4])
      
        box_socres = box_confidence * box_clalss_probs
        box_socres = tf.reshape(box_socres, [-1, classes_num])
    ...
    def _get_feats(self, feats, anchors, num_classes, input_shape):
        ...
        predictions = tf.reshape(feats, [-1, grid_size[0], grid_size[1], num_anchors, num_classes + 5]) # x,y,w,h,class ??????????
